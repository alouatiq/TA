...
Features
--------
• Keeps your previous 7-category workflow (crypto, forex, equities, commodities, futures, warrants, funds)
• Adds a new "Analyze a specific asset" path (pick symbol + indicators to run)
• Passes market/region context from terminal_ui to the data fetchers
• Prints diagnostics (used/failed/skipped sources), basic P&L table, and persists the session

This file intentionally stays thin; heavy lifting is done in trading_core/* modules.
"""
...
from .terminal_ui import (
    ...
    get_user_choice,                  # -> 'crypto'|'forex'|'equities'|'commodities'|'futures'|'warrants'|'funds'
    get_user_budget,
    get_market_selection_details,     # -> {market, region?}, may be None on old UIs
    prompt_single_asset_input,        # -> {symbol, asset_class, market?, region?, timeframes?, indicators?}
    print_header, print_table, print_kv, print_line,
)

# Core: config + data + strategy
...
from trading_core.strategy.rules_engine import (
    analyze_market_batch,      # (rows, market_ctx, feature_flags) -> recommendations
    analyze_single_asset,      # (row, asset_class, market_ctx, feature_flags) -> recommendation
)
...
# Detect local timezone (fallback to UTC-ish)
try:
...
def _feature_flags(use_rsi: bool, use_sma: bool, use_sentiment: bool,
                   custom_indicators: Optional[List[str]] = None) -> Dict[str, Any]:
    """
    Standardize feature flags passed to strategy.
    """
    return {
        "use_rsi": use_rsi,
        "use_sma": use_sma,
        "use_sentiment": use_sentiment,
        "custom_indicators": custom_indicators or [],  # list like ["EMA","MACD","ADX","OBV","BBANDS","ATR",...]
    }


def _print_recommendations(recommendations: List[Dict[str, Any]], *, title: str) -> None:
    """
    Pretty-print a uniform recommendations table.
    """
    print_header(title)
    rows = []
    total_invest = 0.0
...
def run_category_flow() -> None:
    """
    Original 7-category workflow (now backed by new data_fetcher facade).
    """
    # Yesterday’s report
    ...
    include_history = bool(use_rsi or use_sma)
    ...
    feat = _feature_flags(use_rsi, use_sma, use_sentiment)
    try:
    ...
    log_trade(
        ...
        features={"RSI": use_rsi, "SMA": use_sma, "Sentiment": use_sentiment},
    )
...
def run_single_asset_flow() -> None:
    """
    New workflow to analyze one specific symbol/coin/pair with chosen indicators.
    """
    ...
    timeframes   = params.get("timeframes", ["1d"])      # Future use (MTF)
    ...
    # Feature flags: enable only what the user selected (RSI/SMA toggles still respected)
    # Here we assume the single-asset prompt returns booleans for RSI/SMA/Sentiment
    use_rsi       = params.get("use_rsi", "RSI" in indicators)
    use_sma       = params.get("use_sma", "SMA" in indicators or "EMA" in indicators)
    use_sentiment = params.get("use_sentiment", False)

    feat = _feature_flags(use_rsi, use_sma, use_sentiment, custom_indicators=indicators)

    # Market context
    market_ctx = _merge_market_context({"market": market, "region": region} if market or region else None,
                                       asset_class or "asset")

    include_history = bool(use_rsi or use_sma or indicators)
    ...
    log_trade(
        ...
        budget=budget,
        recommendations=[rec],
        features={"RSI": use_rsi, "SMA": use_sma, "Sentiment": use_sentiment, "Custom": indicators},
    )
...
# ────────────────────────────────────────────────────────────
def main() -> None:
    # Ensure markets config is loadable (for sessions/tz/labels)
    try:
        load_markets_config()  # warms up config cache / validates YAML
    except Exception as e:
